...
class Word2Vec(utils.SaveLoad):
    ...
    def most_similar_all(self, positive=[], negative=[], topn=10, restrict_vocab=None):
        self.init_sims()
    ...
    def most_similar_euclidean(self, positive=[], negative=[], topn=10, restrict_vocab=None):
        self.init_sims()
        ...
        limited = self.syn0norm if restrict_vocab is None else self.syn0norm[:restrict_vocab]
        dists = [1/numpy.linalg.norm(indexi-mean) for indexi in limited]
        if not topn:
            return dists
        best = matutils.argsort(dists, topn=topn + len(all_words), reverse=True)
        # ignore (don't return) words from the input
    ...
    def similarity_euclidean(self, w1, w2):
        return 1/numpy.linalg.norm(self[w1] - self[w2])

    def similarity_euclidean_norm(self, w1, w2):
        return 1/numpy.linalg.norm(matutils.unitvec(self[w1]) - matutils.unitvec(self[w2]))
