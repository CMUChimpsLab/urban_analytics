...
var PublicModule = (function() {
    ...
    var Module = {
        ...
        getNghds: function() { return neighborhoods; },
        neighborhoodNames: function() {return neighborhoodNames;},
        ...
        tweetsByThisNghdUsersQuery: function() {
            ...
            $.getJSON("/tweets_by_this_nghd_users", params, function(tweets) {
                ...
                // count the tweets in each neighborhood (zero out first)
                for (var i = 0; i < neighborhoods.features.length; i++) {
                    neighborhoods.features[i].properties.numTweets = 0;
                }
                for (var i = 0; i < allTweets.length; i++) {
                    var nghdName = neighborhoodName(allTweets[i].coordinates.coordinates);
                    if (nghdName != "Outside Pittsburgh") {
                        var nghd = neighborhoods.features[neighborhoodNames.indexOf(nghdName)];
                        nghd.properties.numTweets += 1;
                    }
                }
